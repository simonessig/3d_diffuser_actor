...
def make_request(image, condition):
    ...
    text = "You are given an image of an robotic manipulation task. \
            ...
            The task has an additional constraint which you should define based on the image and some condition. \
            ...
            Make the region not cover up parts of the image where the robots should go. \
            Only output the points and the direction without additional text. \
            Split the points and direction with ; \
...
def create_mask(rect_points, size, direction):
    ...
    # print(dir_mask.shape)
    dir_mask = torch.flip(dir_mask, [0])
    # print(dir_mask.shape)

    small_mask = einops.rearrange(torch.tensor(dir_mask), "h w d -> 1 d w h")
